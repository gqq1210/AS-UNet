...
'''
from uNet.layer import (conv2d, deconv2d, max_pooling_2x2, crop_and_concat, weight_xavier_init, bias_variable, conv_sigmoid, conv_bn_relu_drop, deconv_relu, conv_sigmoid)
...
def squeeze_excitation_model(x, out_dim, name='csse', ratio=4, scope=None):
    with tf.name_scope(scope):
        if name == 'ssce':
