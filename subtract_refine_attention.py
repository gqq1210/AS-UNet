...
def attention_layer(x, ratio=4, scope=None):
    with tf.name_scope(scope):
        ...
        recalibrate3 = conv_sigmoid(x, kernal=(3, 3, channel, channel // 2), scope=scope + 'conv1')
        recalibrate3 = max_pooling_2x2(recalibrate3)
        recalibrate3 = conv_sigmoid(recalibrate3, kernal=(3, 3, channel // 2, channel // 4), scope=scope + 'conv2')
        recalibrate3 = max_pooling_2x2(recalibrate3)
        ...
        deconv = tf.nn.conv2d_transpose(recalibrate3, W, output_shape, strides=[1, 4, 4, 1], padding="SAME") + B
        ...
        attention = tf.multiply(alpha_attention * recalibrate4, beta_attention * recalibrate3)
...
class unet2dModule(object):
    ...
    def __init__(self, image_height, image_width, channels=1, costname="boundary_dice"):
        ...
        self.hausdorff = self.__get_Hausdorff()
    ...
    def __get_cost(self, cost_name):
        ...
        if cost_name == "boundary_dice":
            ...
            loss1_2 = beta * self.dice(self.Y_gt, self.Y_pred)
            loss2_1 = alpha * self.boundary_loss(self.Y_gt, self.output)
    ...
    def __get_Hausdorff(self):
        H, W, C = self.Y_gt.get_shape().as_list()[1:]
        pred_flat = tf.reshape(self.Y_pred, [-1, H * W * C])
        true_flat = tf.reshape(self.Y_gt, [-1, H * W * C])
        a = self.__get_accuracy()
        return a / (2 - a)
    ...
    def train(self, train_images, train_labels, train_edges, model_path, logs_path, learning_rate,
        ...
        train_op = tf.train.AdamOptimizer(self.lr).minimize(self.cost[0])

        init = tf.global_variables_initializer()
        saver = tf.train.Saver(tf.all_variables(), max_to_keep=8)

        tf.summary.scalar("loss", self.cost[0])
        ...
        for i in range(train_epochs):
            ...
            all, train_accuracy, train_hausdorff = sess.run([self.cost, self.accuracy, self.hausdorff], feed_dict={self.X: batch_xs,
                                                                                         self.Y_gt: batch_ys,
            ...
            '''
            print('epochs %d training_loss ,Training_accuracy ,Train_hausdorff => %.5f,%.5f,%.5f ' % (i, train_loss, train_accuracy, train_accuracy / (2-train_accuracy)))
